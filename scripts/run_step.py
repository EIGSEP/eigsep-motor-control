...
parser.add_argument('-i', dest = 'inf_', type = bool, default=0, help = 'Setting to rotate stepper motor infinitely')
...
def main_function(pins, d, angle, name, inf=False):
    ...
    inf: bool
        The option to have the rotations of the stepper motor continue infinitely
    """
    try:
        motor = Stepper(pins)
...
def both(name1, name2):
    ...
    None
    """
    t1 = threading.Thread(target=main_function, args=(elevation_pins, delay, box_angle, name1, inf))
    t2 = threading.Thread(target=main_function, args=(azimuth_pins, delay, antenna_angle, name2, inf))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
